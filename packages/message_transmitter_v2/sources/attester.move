...
module message_transmitter_v2::attester {
    // Built-in Modules
    use std::error;
    use std::signer;
    use std::event;
    ...
    public fun is_enabled_attester(attester: address): bool {
        ...
        enabled_attesters.contains(&attester)
    }
    ...
    public fun get_enabled_attester(index: u64): address {
        let enabled_attesters = state::get_enabled_attesters();
        assert!(index < enabled_attesters.length(), error::invalid_argument(EINVALID_INDEX));
        enabled_attesters[index]
    }
    ...
    entry fun disable_attester(caller: &signer, attester: address) {
        assert_is_attester_manager(caller);
        let enabled_attesters = state::get_enabled_attesters();
        assert!(enabled_attesters.length() > 1, error::invalid_state(ETOO_FEW_ENABLED_ATTESTERS));
        assert!(
            enabled_attesters.length() > state::get_signature_threshold(),
    ...
    /// - the new attester manager is zero address
    /// - the new attester manager is the same as the current attester manager
    entry fun update_attester_manager(caller: &signer, new_attester_manager: address) {
        ...
        assert!(new_attester_manager != @0x0, error::invalid_argument(EINVALID_ATTESTER_MANAGER));
    ...
    public fun verify_attestation_signature(message: &vector<u8>, attestation: &vector<u8>) {
        ...
        assert!(
            attestation.length() == SIGNATURE_LENGTH * signature_threshold,
        ...
        for (i in 0..signature_threshold) {
            ...
            let signature_with_recovery_id = attestation.slice(
                i * SIGNATURE_LENGTH,
            ...
            assert!(result.is_greater_than(), error::invalid_argument(EATTESTER_NOT_IN_INCREASING_ORDER));
    ...
    package fun init_attester(caller: &signer, attester: address) {
    ...
    fun recover_attester_address(signature: &vector<u8>, message_digest: &vector<u8>): address {
        ...
        let v_value = signature[SIGNATURE_LENGTH - 1];
        assert!(v_value == 27 || v_value == 28, error::invalid_argument(EINVALID_SIGNATURE));
        let recovery_id = v_value - 27;

        // Recover public key
        let ecdsa_signature = secp256k1::ecdsa_signature_from_bytes(
            signature.slice(0, SIGNATURE_LENGTH - 1)
        );
        let recovered_public_key = secp256k1::ecdsa_recover(*message_digest, recovery_id, &ecdsa_signature);
        assert!(recovered_public_key.is_some(), error::invalid_argument(EINVALID_MESSAGE_OR_SIGNATURE));

        // Convert public key to address
        let recovered_attester_address = get_address_from_public_key(
            &secp256k1::ecdsa_raw_public_key_to_bytes(recovered_public_key.borrow())
        );
    ...
    fun verify_low_s_value(signature: &vector<u8>): bool {
        ...
        let signature_s_value = signature.slice( 32, SIGNATURE_LENGTH-1);
        let result = comparator::compare(&signature_s_value, &HALF_CURVE_ORDER);
        result.is_smaller_than() || result.is_equal()
    }
    ...
    fun get_address_from_public_key(public_key: &vector<u8>): address {
        ...
        let address_without_prefix = address_bytes.slice(
            address_bytes.length() - 20,
            address_bytes.length()
        );

        // Add 0x0 prefix to make the address 32 bytes
        let address_with_prefix = x"000000000000000000000000";
        address_with_prefix.append(address_without_prefix);
    ...
    fun test_init_attester(owner: &signer) {
        ...
        assert_eq(state::get_num_enabled_attesters(), 1);
        assert_eq(state::get_enabled_attesters()[0], attester);
    ...
    #[test(owner = @message_transmitter_v2, caller = @0xfab)]
    #[expected_failure(abort_code = 0x50005, location = Self)]
    ...
    fun test_enable_attester_success(owner: &signer) {
        ...
        assert!(state::get_enabled_attesters().contains(&new_attester), 0);

        let attester_enabled_event = AttesterEnabled { attester: new_attester };
        assert!(event::was_event_emitted(&attester_enabled_event), 0);
    }

    #[test(owner = @message_transmitter_v2, not_attester_manager =  @0x99)]
    #[expected_failure(abort_code = 0x50005, location = Self)]
    ...
    #[test(owner = @message_transmitter_v2, not_owner = @0x99)]
    #[expected_failure(abort_code = 0x00002, location = ownable)]
    ...
    fun test_update_attester_manager_zero_address(owner: &signer) {
        init_for_test(owner, @0xfac);
        update_attester_manager(owner, @0x0);
    }
    ...
    fun test_update_attester_manager_already_exists(owner: &signer) {
        init_for_test(owner, @0xfac);
        update_attester_manager(owner, @message_transmitter_v2);
    }
    ...
    fun test_is_enabled_attester(owner: &signer) {
        init_for_test(owner, @0xfac);
        assert!(is_enabled_attester(@0xfac));
        assert!(!is_enabled_attester(@0xfab));
    }
    ...
    fun test_get_num_enabled_attester(owner: &signer) {
        ...
        assert_eq(get_num_enabled_attesters(), 2);
    }

    #[test(owner = @message_transmitter_v2)]
    fun test_view_attester_manager(owner: &signer) {
        init_for_test(owner, @0xfac);
        assert_eq(attester_manager(), state::get_attester_manager());
    }
    ...
    #[test(owner = @message_transmitter_v2)]
    fun test_get_enabled_attester_success(owner: &signer) {
        init_for_test(owner, @0xfac);
        enable_attester(owner, @0xfab);
        assert_eq(get_enabled_attester(0), @0xfac);
        assert_eq(get_enabled_attester(1), @0xfab);
    }

    #[test(owner = @message_transmitter_v2)]
    #[expected_failure(abort_code = 0x10001, location = Self)]
    fun test_get_enabled_attester_invalid_index(owner: &signer) {
        init_for_test(owner, @0xfac);
        get_enabled_attester(1);
    }
}
