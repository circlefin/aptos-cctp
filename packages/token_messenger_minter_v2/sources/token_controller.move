...
module token_messenger_minter_v2::token_controller {
    ...
    const EINVALID_TOKEN_CONTROLLER_ADDRESS: u64 = 6;
    ...
    package entry fun set_token_controller(caller: &signer, new_token_controller: address) {
        assert!(new_token_controller != @0x0, error::invalid_argument(EINVALID_TOKEN_CONTROLLER_ADDRESS));
    ...
    package fun assert_amount_within_burn_limit(token: address, amount: u64) {
    ...
    fun test_set_token_controller(owner: &signer) {
        ...
        set_token_controller(owner, token_controller);
        assert_eq(state::get_token_controller(), token_controller);
        assert!(event::was_event_emitted(&SetTokenController { token_controller }), 0);
    }

    #[test(owner = @deployer)]
    #[expected_failure(abort_code = 0x10006, location = Self)]
    fun test_set_token_controller_zero_address(owner: &signer) {
        init_test_token_controller(owner);
        set_token_controller(owner, @0x0);
    }
    ...
    fun test_get_local_token_no_link_exists(owner: &signer) {
        init_test_token_controller(owner);
        // Call get_local_token without linking any token - should abort with ENO_LINK_EXIST_FOR_REMOTE_DOMAIN_AND_TOKEN
        get_local_token(4, @0xfac);
    }
    ...
    fun test_view_functions(owner: &signer) {
        init_test_token_controller(owner);
        assert_eq(token_controller(), state::get_token_controller());
        assert_eq(get_num_linked_tokens(), 0);
