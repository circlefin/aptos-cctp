...
// Example: Transfer 1 USDC from Aptos to EVM with hook data attached.
// Hook data is forwarded to the destination chain and can be used for post-mint execution.
const main = async () => {
  ...
  // STEP 1: Build and submit deposit-for-burn-with-hook transaction on Aptos
  const bytecode = Uint8Array.from(
    readFileSync("typescript/example/precompiled-move-scripts/testnet/v2/deposit_for_burn_with_hook.mv")
  );
  const hookData = Buffer.from("example hook data"); // must be non-empty
  const functionArguments: Array<any> = [
    ...
    MoveVector.U8(hookData),                       // hookData
  ];
