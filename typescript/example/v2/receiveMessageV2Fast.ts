...
const MAX_FEE = 100;           // max fee deduction accepted from transferred amount (in token smallest units)

// Example: Transfer 1 USDC from EVM to Aptos (fast transfer, lower finality threshold)
const main = async () => {
  ...
  const burnTxGas = await tokenMessengerContract.methods
    .depositForBurn(USDC_AMOUNT, APTOS_DOMAIN, mintRecipient, EVM_USDC_ADDRESS, destinationCaller, MAX_FEE, FINALITY_THRESHOLD_CONFIRMED)
    .estimateGas();
